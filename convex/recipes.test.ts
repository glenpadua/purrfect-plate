...
describe("private shared recipe library", () => {
  ...
  test("editing grouped recipes preserves untouched source evidence and publisher notes", async () => {
    ...
    const id = await glen.mutation(api.recipes.create, {
      ...
      recipeNotes: [{ text: "Use long grain rice.", sourceIds: ["card"] }],
    });
    await millusha.mutation(api.recipes.update, {
      id,
    ...
    await glen.mutation(api.recipes.update, {
      id,
      ingredients: recipeLinesFromText("## Rice\n2 cups rice", original),
      recipeNotes: [],
    });
    const edited = await millusha.query(api.recipes.get, { id });
    expect(edited?.ingredients?.map(({ quantity: _quantity, ...line }) => line)).toEqual([
      { text: "2 cups rice", group: "Rice" },
    ]);
  ...
  test("creates structured manual recipes, normalizes tags, and shares between members", async () => {
    ...
    const id = await glen.mutation(api.recipes.create, {
      ...
      instructions: [{ text: "Cook the rice." }],
  ...
  test("all public operations reject signed-out callers", async () => {
    ...
    await expect(t.mutation(api.recipes.create, { name: "No", tags: [] })).rejects.toThrow(
      "Sign in",
    );
  ...
  test("invites require verified emails and cannot be claimed by another address", async () => {
    ...
    await expect(
      t
        .withIdentity({ subject: "fake", email: "glen@example.com", emailVerified: false })
        .mutation(api.libraries.join, {}),
    ).rejects.toThrow("verified");
    await expect(
      t
        .withIdentity({ subject: "outsider", email: "outsider@example.com", emailVerified: true })
        .mutation(api.libraries.join, {}),
  ...
  test("another library cannot read, change, or delete these recipes", async () => {
    ...
    await expect(other.mutation(api.recipes.update, { id, name: "Stolen" })).rejects.toThrow(
      "not found",
    );
  ...
  test("search, tags, and pagination stay inside the shared library", async () => {
    ...
    expect(
      (await glen.query(api.recipes.list, { search: "omelette", tags: ["dinner"] })).map(
        (r) => r.name,
      ),
    ).toEqual(["Omelette"]);
    expect(await glen.query(api.recipes.listTags, {})).toEqual(["breakfast", "comfort", "dinner"]);
    const first = await glen.query(api.recipes.listPage, {
      paginationOpts: { numItems: 1, cursor: null },
    });
    const second = await glen.query(api.recipes.listPage, {
  ...
  test("new image storage is capped and belongs to its uploader", async () => {
    ...
    await expect(
      millusha.mutation(api.recipes.create, { name: "No", tags: [], imageStorageId: storageId }),
    ...
    await expect(glen.mutation(api.recipes.registerUpload, { storageId })).rejects.toThrow(
      "unavailable",
    );
    await expect(
      glen.mutation(api.recipes.create, {
    ...
    const large = await t.run((ctx) =>
      ctx.storage.store(new Blob([new Uint8Array(350001)], { type: "image/webp" })),
    );
    await expect(glen.mutation(api.recipes.registerUpload, { storageId: large })).rejects.toThrow(
      "350 KB",
    );
  });
  test("rejects empty names and unreasonable recipe sizes", async () => {
    const { glen } = await setup();
    await expect(glen.mutation(api.recipes.create, { name: " ", tags: [] })).rejects.toThrow(
      "name",
    );
    await expect(
      glen.mutation(api.recipes.create, {
        name: "Huge",
        tags: [],
        ingredients: Array.from({ length: 101 }, () => ({ text: "Salt" })),
      }),
    ).rejects.toThrow("100");
  });
});

test("deleting a recipe removes its pantry bindings and preferences but keeps shared ingredients", async () => {
  const { t, glen } = await setup();
  const id = await glen.mutation(api.recipes.create, {
    name: "Dinner",
    tags: [],
    servings: "2",
    ingredients: [{ text: "salt or pepper" }],
  });
  const other = await glen.mutation(api.recipes.create, {
    name: "Other",
    tags: [],
    ingredients: [{ text: "salt or pepper" }],
  });
  for (const recipeId of [id, other])
  ...
  expect((await glen.query(api.pantry.matches, { recipeId: other }))[0]).toMatchObject({
    chosen: true,
    present: true,
    names: ["salt"],
  });
