...
async function setup() {
  ...
  await glen.mutation(api.libraries.join, {}); await millusha.mutation(api.libraries.join, {})
  const ingredients = [{ text: "Mutton- 1 kg", sourceIds: ["source"] }, { text: "Rice- 700 gms (4 US cups)" }]
  const id = await glen.mutation(api.recipes.create, { name: "Biryani", servings: "Serves 4", tags: [], ingredients })
...
test("base edits are shared while source quantities and source yield are preserved", async () => {
  ...
  expect(await millusha.query(api.recipes.get, { id })).toMatchObject({ servings: "Serves 4", servingInfo: { count: 6, origin: "user" }, ingredients })
  for (const count of [0, 1.5, 101, NaN, Infinity]) await expect(glen.mutation(api.recipes.setBaseServings, { id, count })).rejects.toThrow()
})
test("same account on another device resumes its adjustment, other members keep their own", async () => {
  ...
  const preference = { adjustment: { mode: "ingredient" as const, ingredientText: "Mutton- 1 kg", amount: 500, unit: "g" }, units: "metric" as const }
  ...
  await millusha.mutation(api.recipes.setCookingPreference, { id, preference: { adjustment: { mode: "servings", servings: 8 }, units: "original" } })
  await glen.mutation(api.recipes.setCookingPreference, { id, preference: { adjustment: { mode: "original" }, units: "original" } })
  expect(await millusha.query(api.recipes.getCookingPreference, { id })).toMatchObject({ adjustment: { servings: 8 } })
...
test("old recipes without servings automatically acquire estimates on read without source rewrites", async () => {
  ...
  const id = await glen.mutation(api.recipes.create, { name: "Rice", tags: [], ingredients: [{ text: "360 g rice" }] })
  expect(await glen.query(api.recipes.get, { id })).toMatchObject({ servingInfo: { count: 4, origin: "estimated" } })
  ...
  expect(await glen.query(api.recipes.get, { id })).toMatchObject({ servingInfo: { count: 2, origin: "estimated" } })
})
test("preference writes reject stale ingredients, incompatible units and invalid factors", async () => {
  ...
  for (const adjustment of [
    ...
    { mode: "ingredient" as const, ingredientText: "Other recipe ingredient", amount: 500, unit: "g" },
    { mode: "servings" as const, servings: -1 }, { mode: "servings" as const, servings: Infinity },
  ]) await expect(glen.mutation(api.recipes.setCookingPreference, { id, preference: { adjustment, units: "original" } })).rejects.toThrow()
})
test("sign-in and recipe library membership are required for every serving operation", async () => {
  ...
  for (const caller of [t, t.withIdentity({ subject: "other" })]) {
    ...
    await expect(caller.mutation(api.recipes.setCookingPreference, { id, preference: { adjustment: { mode: "original" }, units: "original" } })).rejects.toThrow()
  }
...
test("saving the full editor re-estimates changed amounts without losing a user correction", async () => {
  ...
  const id = await glen.mutation(api.recipes.create, { name: "Rice", tags: [], ingredients: [{ text: "360 g rice" }] })
  await glen.mutation(api.recipes.update, { id, ingredients: [{ text: "180 g rice" }], servingInfo: { count: 4, origin: "estimated" } })
  expect(await glen.query(api.recipes.get, { id })).toMatchObject({ servingInfo: { count: 2, origin: "estimated" } })
  await glen.mutation(api.recipes.update, { id, ingredients: [{ text: "360 g rice" }], servingInfo: { count: 3, origin: "user" } })
  expect(await glen.query(api.recipes.get, { id })).toMatchObject({ servingInfo: { count: 3, origin: "user" } })
})
