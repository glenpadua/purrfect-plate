...
export default defineSchema({
  recipeCookingPreferences: defineTable({
    recipeId: v.id("recipes"),
    userId: v.string(),
    ...cookingPreference.fields,
  }).index("by_recipe_user", ["recipeId", "userId"]),
  migrationRecords: defineTable({ legacyId: v.string(), recipeId: v.id("recipes") }).index(
    "by_legacy",
    ["legacyId"],
  ),
  libraries: defineTable({
    ...
    slug: v.string(),
    createdAt: v.number(),
    pantryVersion: v.optional(v.number()),
  }).index("by_slug", ["slug"]),
  memberships: defineTable({
    ...
    .index("by_library_user", ["libraryId", "userId"]),
  libraryInvites: defineTable({
    libraryId: v.id("libraries"),
    email: v.string(),
    role: v.union(v.literal("owner"), v.literal("member")),
  ...
  pantryItems: defineTable({
    ...
    present: v.boolean(),
    updatedAt: v.number(),
    mergedInto: v.optional(v.id("pantryItems")),
  })
  ...
  ingredientAliases: defineTable({
    libraryId: v.id("libraries"),
    key: v.string(),
    ingredientId: v.id("pantryItems"),
  }).index("by_library_key", ["libraryId", "key"]),
  recipeIngredientBindings: defineTable({
    libraryId: v.id("libraries"),
    recipeId: v.id("recipes"),
    text: v.string(),
  ...
  importUsage: defineTable({
    ...
    day: v.string(),
    count: v.number(),
  }).index("by_library_day", ["libraryId", "day"]),
  imports: defineTable({
    libraryId: v.id("libraries"),
    createdBy: v.string(),
    url: v.string(),
