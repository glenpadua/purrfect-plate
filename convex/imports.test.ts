...
const draft = {
  ...
  instructions: [{ text: "Stir gently." }],
  warnings: [],
};
beforeEach(() => {
  vi.useFakeTimers();
  vi.stubEnv("IMPORT_WORKER_SECRET", "test-worker");
});
afterEach(() => {
  vi.useRealTimers();
  vi.unstubAllEnvs();
...
test("canonical links share one job and repeated save creates one recipe", async () => {
  ...
  const id = await user.mutation(api.imports.start, {
    url: "https://www.youtube.com/shorts/_qFZJjnN73o?si=abc",
  });
  expect(
    await user.mutation(api.imports.start, { url: "https://www.youtube.com/watch?v=_qFZJjnN73o" }),
  ).toBe(id);
  ...
  expect((await user.query(api.imports.get, { id }))?.draft?.ingredients[0]).toMatchObject({
    text: "2 eggs",
    quantity: { status: "scalable", parsed: { amount: 2 } },
  });
  const recipeId = await user.mutation(api.imports.save, { id, draft });
  expect((await t.run((ctx) => ctx.db.get(recipeId)))?.ingredients?.[0]).toMatchObject({
    text: "2 eggs",
    quantity: { status: "scalable", parsed: { amount: 2 } },
  });
...
test("failed imports can be cleared, restored, and reopened without resetting retry limits", async () => {
  const { t, user } = await setup();
  const url = "https://example.com/clear-failed";
  const id = await user.mutation(api.imports.start, { url });
  ...
  const before = await user.query(api.imports.get, { id });
  await user.mutation(api.imports.setDismissed, { id, dismissed: true });
  expect(await user.query(api.imports.list, {})).toEqual([]);
  expect(await user.query(api.imports.get, { id })).toEqual(before);
  await user.mutation(api.imports.setDismissed, { id, dismissed: false });
  expect(await user.query(api.imports.list, {})).toHaveLength(1);
  await user.mutation(api.imports.setDismissed, { id, dismissed: true });
  expect(await user.mutation(api.imports.start, { url })).toBe(id);
  expect(await user.query(api.imports.list, {})).toHaveLength(1);
  await expect(user.mutation(api.imports.retry, { id })).rejects.toThrow("Wait 15 minutes");
  expect(await t.run(async (ctx) => (await ctx.db.query("importUsage").collect())[0].count)).toBe(
    3,
  );
});
test("clearing enforces library ownership and protects running, reviewable, and saved imports", async () => {
  const { t, user } = await setup();
  const id = await user.mutation(api.imports.start, { url: "https://example.com/clear-guards" });
  await expect(t.mutation(api.imports.setDismissed, { id, dismissed: true })).rejects.toThrow(
    "Sign in",
  );
  await expect(user.mutation(api.imports.setDismissed, { id, dismissed: true })).rejects.toThrow(
    "Only failed",
  );
  ...
  for (const dismissed of [true, false])
    await expect(other.mutation(api.imports.setDismissed, { id, dismissed })).rejects.toThrow(
      "Import not found",
    );
  await user.mutation(api.imports.setDismissed, { id, dismissed: true });
  ...
  await user.mutation(api.imports.save, { id, draft });
  await expect(user.mutation(api.imports.setDismissed, { id, dismissed: true })).rejects.toThrow(
    "Only failed",
  );
  expect(await user.query(api.recipes.list, {})).toHaveLength(1);
...
test("auth, worker secret and concurrency limits are enforced", async () => {
  const { t, user } = await setup();
  await expect(
    t.mutation(api.imports.start, { url: "https://example.com/recipe" }),
  ...
  await expect(user.mutation(api.imports.start, { url: "https://example.com/3" })).rejects.toThrow(
    "Two imports",
  );
  await expect(
    t.mutation(api.imports.workerFinish, { secret: "wrong", id, attempt: 1, draft }),
...
test("long import warnings do not prevent subsequent recipe edits", async () => {
  ...
  await t.mutation(api.imports.workerFinish, {
    secret: "test-worker",
    id,
    attempt: 1,
    draft: withWarnings,
...
test("wrong link shapes never create a job, and partial-source failures keep their recovery hint", async () => {
  const { t, user } = await setup();
  await expect(
    user.mutation(api.imports.start, { url: "https://youtube.com/@chef" }),
  ).rejects.toThrow("specific");
  expect(await user.query(api.imports.list, {})).toEqual([]);
  const id = await user.mutation(api.imports.start, {
    url: "https://youtube.com/shorts/GiqOJyy3oWE",
  });
  await t.mutation(internal.imports.claim, { id, attempt: 1 });
  await t.mutation(api.imports.workerFinish, {
    secret: "test-worker",
    id,
    attempt: 1,
    error: "No readable recipe details.",
    failureCode: "insufficient",
    searchQuery: "biryani recipe",
  });
  expect(await user.query(api.imports.get, { id })).toMatchObject({
    status: "failed",
    failureCode: "insufficient",
    searchQuery: "biryani recipe",
  });
...
test("members can override only a relevance decision, with the same durable job", async () => {
  ...
  await t.mutation(api.imports.workerFinish, {
    ...
    failureCode: "not_recipe",
  });
  await expect(t.mutation(api.imports.retry, { id, continueAnyway: true })).rejects.toThrow(
    "Sign in",
  );
  await user.mutation(api.imports.retry, { id, continueAnyway: true });
  await t.mutation(internal.imports.claim, { id, attempt: 2 });
  expect(
    await t.query(api.imports.workerGet, { secret: "test-worker", id, attempt: 2 }),
  ).toMatchObject({ relevanceOverride: true });
  await t.mutation(api.imports.workerFinish, {
    secret: "test-worker",
    id,
    attempt: 2,
  ...
  await expect(user.mutation(api.imports.retry, { id, continueAnyway: true })).rejects.toThrow(
    "Only a relevance",
  );
});
test("members can explicitly recheck an unsaved draft without duplicating it or replacing a saved recipe", async () => {
  const { t, user } = await setup();
  const id = await user.mutation(api.imports.start, {
    url: "https://example.com/recipe-to-recheck",
  });
  await t.mutation(internal.imports.claim, { id, attempt: 1 });
  await t.mutation(api.imports.workerFinish, { secret: "test-worker", id, attempt: 1, draft });
  const previous = (await user.query(api.imports.get, { id }))!;
  await expect(
    t.mutation(api.imports.recheckDraft, { id, expectedUpdatedAt: previous.updatedAt }),
  ).rejects.toThrow("Sign in");
  await expect(
    user.mutation(api.imports.recheckDraft, { id, expectedUpdatedAt: previous.updatedAt - 1 }),
  ...
  expect(
    await user.mutation(api.imports.start, { url: "https://example.com/recipe-to-recheck" }),
  ).toBe(id);
  await t.mutation(internal.imports.claim, { id, attempt: 2 });
  await t.mutation(api.imports.workerFinish, { secret: "test-worker", id, attempt: 2, draft });
  const recipeId = await user.mutation(api.imports.save, { id, draft });
  ...
  await expect(
    user.mutation(api.imports.recheckDraft, { id, expectedUpdatedAt: saved.updatedAt }),
