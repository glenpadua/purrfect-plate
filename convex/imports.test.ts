...
test("canonical links share one job and repeated save creates one recipe", async () => {
  ...
  expect((await user.query(api.imports.get, { id }))?.draft?.ingredients[0]).toMatchObject({ text: "2 eggs", quantity: { status: "scalable", parsed: { amount: 2 } } })
  const recipeId = await user.mutation(api.imports.save, { id, draft })
  expect((await t.run(ctx => ctx.db.get(recipeId)))?.ingredients?.[0]).toMatchObject({ text: "2 eggs", quantity: { status: "scalable", parsed: { amount: 2 } } })
...
test("failed imports can be cleared, restored, and reopened without resetting retry limits", async () => {
  ...
  expect(await t.run(async ctx => (await ctx.db.query("importUsage").collect())[0].count)).toBe(3)
})
test("clearing enforces library ownership and protects running, reviewable, and saved imports", async () => {
  ...
  await expect(t.mutation(api.imports.setDismissed, { id, dismissed: true })).rejects.toThrow("Sign in")
  await expect(user.mutation(api.imports.setDismissed, { id, dismissed: true })).rejects.toThrow("Only failed")
  await t.mutation(internal.imports.claim, { id, attempt: 1 })
  await expect(user.mutation(api.imports.setDismissed, { id, dismissed: true })).rejects.toThrow("Only failed")
  await t.mutation(internal.imports.expire, { id, attempt: 1 })
  await t.run(async ctx => {
    const libraryId = await ctx.db.insert("libraries", { name: "Other", slug: "other", createdAt: Date.now() })
    await ctx.db.insert("memberships", { libraryId, userId: "other", email: "other@example.com", role: "owner", createdAt: Date.now() })
  })
  const other = t.withIdentity({ subject: "other" })
  for (const dismissed of [true, false]) await expect(other.mutation(api.imports.setDismissed, { id, dismissed })).rejects.toThrow("Import not found")
  await user.mutation(api.imports.setDismissed, { id, dismissed: true })
