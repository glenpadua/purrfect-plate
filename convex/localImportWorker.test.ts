...
test("local polling claims once, needs the worker secret, and cannot claim production jobs", async () => {
  ...
  await expect(t.mutation(api.localImportWorker.claimNext, { secret: "wrong" })).rejects.toThrow(
    "Unauthorized",
  );
  vi.stubEnv("CONVEX_CLOUD_URL", "https://spotted-gazelle-950.convex.cloud");
  await expect(
    t.mutation(api.localImportWorker.claimNext, { secret: "test-secret" }),
  ).rejects.toThrow("Unauthorized");
  vi.stubEnv("CONVEX_CLOUD_URL", "https://basic-poodle-462.convex.cloud");
  expect(await t.mutation(api.localImportWorker.claimNext, { secret: "test-secret" })).toEqual({
    id,
    attempt: 1,
  });
