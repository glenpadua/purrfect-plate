...
test("draft repairs retain source evidence and reject stale or already-saved results", async () => {
  ...
  const evidenceJson = JSON.stringify({
    ...
    evidence: [{ id: "speech", text: "Add flour." }],
  });
  ...
  const args = {
    ...
    draft: {
      ...
      instructions: [{ text: "Add flour." }],
    },
    evidenceJson,
  };
  await expect(
    t.mutation(internal.migrations.repairUnsavedDraft, {
      ...args,
      evidenceJson: evidenceJson.replace("Add flour.", "Bake it."),
    }),
