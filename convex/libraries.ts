...
export const current = query({
  args: {},
  returns: v.union(
    v.object({ id: v.id("libraries"), name: v.string(), role: v.string() }),
    v.null(),
  ),
  handler: async (ctx) => {
...
export const join = mutation({
  ...
  handler: async (ctx) => {
    ...
    const invite = await ctx.db
      .query("libraryInvites")
      .withIndex("by_email", (q) => q.eq("email", email))
      .unique();
    if (!invite)
      throw new ConvexError(
        "This library is currently private. Sign in with your invited email address.",
      );
...
export const bootstrap = internalMutation({
  ...
  returns: v.object({ libraryId: v.id("libraries"), migrated: v.number() }),
  handler: async (ctx, { emails }) => {
    ...
    const libraryId =
      ...
      (await ctx.db.insert("libraries", {
        name: "Our recipe library",
        slug: "glen-and-millusha",
        createdAt: Date.now(),
      }));
    ...
    const legacy = await ctx.db
      .query("recipes")
      .withIndex("by_library_created", (q) => q.eq("libraryId", undefined))
      .take(200);
