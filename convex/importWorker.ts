...
export const dispatch = internalAction({
  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      const response = await fetch(url, { method: "POST", headers: { Authorization: `Bearer ${secret}`, "Content-Type": "application/json" }, body: JSON.stringify(args), signal: AbortSignal.timeout(285_000) })
      ...
      await ctx.runMutation(api.imports.workerFinish, { ...args, secret: process.env.IMPORT_WORKER_SECRET ?? "", error: "The import service could not finish. Please retry; your source link is saved." })
    }
    return null
  },
})
