...
test("Indian aliases share one identity while seeds, ground spices and leaves stay separate", async () => {
  ...
  const cumin = await millusha.mutation(api.pantry.setPresence, { name: "cumin seeds", present: true })
  ...
  expect((await glen.query(api.pantry.page, firstPage)).page.map(item => item.name)).toEqual(["coriander leaves", "coriander seeds", "cumin seeds", "ground cumin"])
...
test("stock and shopping remain independent through every removal and clear/undo", async () => {
  ...
  expect((await glen.query(api.pantry.page, firstPage)).page.map(item => item.id)).toEqual([rice])
})
...
test("recipe checkboxes drive bulk shopping, aliases deduplicate, and unclear source lines are retained", async () => {
  ...
  const ingredients = [{ text: "2 onions, finely chopped", sourceIds: ["publisher"] }, { text: "1 tsp jeera" }, { text: "2 tsp cumin seeds" }, { text: "butter or oil" }]
  const recipeId = await glen.mutation(api.recipes.create, { name: "Dinner", tags: [], ingredients })
  await glen.mutation(api.pantry.setRecipePresence, { recipeId, text: ingredients[0].text, present: true })
  expect((await glen.query(api.pantry.matches, { recipeId })).map(item => item.present)).toEqual([true, false, false, false])
  expect(await glen.mutation(api.pantry.addMissing, { recipeId })).toBe(2)
  expect((await glen.query(api.pantry.shopping, {})).map(item => item.name)).toEqual(["cumin seeds", "butter or oil"])
...
test("ambiguous choices persist per recipe, support combined lines, and never become global aliases", async () => {
  ...
  await expect(glen.mutation(api.pantry.setRecipePresence, { recipeId: first, text: "dhania", present: true })).rejects.toThrow("Choose")
  await glen.mutation(api.pantry.setRecipePresence, { recipeId: first, text: "dhania", present: true, names: ["coriander leaves"] })
  await glen.mutation(api.pantry.setRecipePresence, { recipeId: first, text: "salt and pepper", present: true, names: ["salt", "black pepper"] })
  expect((await millusha.query(api.pantry.matches, { recipeId: first })).map(item => item.present)).toEqual([true, true])
  expect((await glen.query(api.pantry.matches, { recipeId: second })).map(item => item.resolved)).toEqual([false, false])
  await glen.mutation(api.pantry.setRecipePresence, { recipeId: first, text: "salt and pepper", present: false })
  expect((await glen.query(api.pantry.page, firstPage)).page.map(item => item.name)).toEqual(["coriander leaves"])
  expect(await glen.query(api.pantry.coverage, { recipeIds: [first, second] })).toEqual([{ recipeId: first, present: 1, total: 2 }, { recipeId: second, present: 0, total: 2 }])
})

test("rename preserves IDs, previous names, source text, shopping labels and Undo references", async () => {
  const { glen } = await setup()
  const ingredientId = await glen.mutation(api.pantry.setPresence, { name: "jeera", present: true })
  const recipeId = await glen.mutation(api.recipes.create, { name: "Rice", tags: [], ingredients: [{ text: "1 tsp jeera" }] })
  await glen.mutation(api.pantry.addToShopping, { name: "cumin seeds" })
  const cleared = await glen.mutation(api.pantry.clearShopping, {})
  expect(await glen.mutation(api.pantry.rename, { ingredientId, name: "Whole jeera" })).toEqual({ status: "saved" })
...
test("a rename collision previews before merge and combines stock, aliases, shopping and saved recipe choices", async () => {
  const { glen } = await setup()
  const source = await glen.mutation(api.pantry.setPresence, { name: "my special flour", present: true })
  const target = await glen.mutation(api.pantry.setPresence, { name: "chickpea flour", present: false })
  const recipeId = await glen.mutation(api.recipes.create, { name: "Pancakes", tags: [], ingredients: [{ text: "flour or besan" }] })
  await glen.mutation(api.pantry.setRecipePresence, { recipeId, text: "flour or besan", present: true, names: ["my special flour"] })
  ...
  expect(await glen.mutation(api.pantry.rename, { ingredientId: source, name: "besan" })).toEqual({ status: "merge_required", targetId: target, targetName: "chickpea flour" })
  expect(await glen.query(api.pantry.shopping, {})).toHaveLength(2)
  await glen.mutation(api.pantry.rename, { ingredientId: source, name: "besan", mergeInto: target })
  expect((await glen.query(api.pantry.page, firstPage)).page.map(item => item.id)).toEqual([target])
  expect((await glen.query(api.pantry.shopping, {})).map(item => item.ingredientId)).toEqual([target])
  expect((await glen.query(api.pantry.matches, { recipeId }))[0]).toMatchObject({ ingredientIds: [target], present: true })
  expect(await glen.mutation(api.pantry.setPresence, { name: "my special flour", present: true })).toBe(target)
})
...
test("freeform shopping edits stay editable and can link to an existing ingredient without changing stock", async () => {
  ...
  expect(await glen.mutation(api.pantry.renameShopping, { id: row.id, name: "butter" })).toMatchObject({ status: "merge_required", targetId: butter })
...
test("legacy upgrade preserves independent stock and shopping, merges only known equivalents, and is repeatable", async () => {
  ...
  await t.run(async ctx => {
    ...
    await ctx.db.insert("pantryItems", { libraryId, key: "jeera", name: "jeera", present: true, updatedAt: 1 })
    await ctx.db.insert("pantryItems", { libraryId, key: "cumin seeds", name: "cumin seeds", present: false, updatedAt: 2 })
    await ctx.db.insert("pantryItems", { libraryId, key: "rice", name: "rice", present: false, updatedAt: 3 })
...
test("pantry pagination and search work beyond the old 300-item ceiling", async () => {
  ...
  await t.run(async ctx => {
    const libraryId = (await ctx.db.query("libraries").first())!._id
    for (let i = 0; i < 320; i++) await ctx.db.insert("pantryItems", { libraryId, key: `item ${i}`, name: `Ingredient ${String(i).padStart(3, "0")}`, present: true, updatedAt: i })
  })
  let result = await glen.query(api.pantry.page, firstPage)
  const ids = new Set(result.page.map(item => item.id))
  while (!result.isDone) {
    result = await glen.query(api.pantry.page, { paginationOpts: { numItems: 60, cursor: result.continueCursor } })
  ...
  expect((await glen.query(api.pantry.page, { ...firstPage, search: "rice" })).page.map(item => item.name)).toEqual(["rice"])
})
...
test("signed-out and other libraries cannot read, rename, merge, bind, clear, or restore this kitchen", async () => {
  ...
  const recipeId = await glen.mutation(api.recipes.create, { name: "Rice", tags: [], ingredients: [{ text: "rice" }] })
  await glen.mutation(api.pantry.addToShopping, { name: "rice" })
  ...
  await expect(other.mutation(api.pantry.rename, { ingredientId, name: "stolen" })).rejects.toThrow("not found")
  await expect(other.mutation(api.pantry.removeShopping, { id: row.id })).rejects.toThrow("not found")
  await expect(other.mutation(api.pantry.renameShopping, { id: row.id, name: "other" })).rejects.toThrow("not found")
  await expect(other.mutation(api.pantry.setRecipePresence, { recipeId, text: "rice", present: true, names: ["rice"] })).rejects.toThrow("not found")
  await expect(other.mutation(api.pantry.restoreShopping, { items: [{ name: "rice", ingredientId }] })).rejects.toThrow("not found")
  expect(await glen.query(api.pantry.shopping, {})).toHaveLength(1)
})

test("failed batches roll back, and stale recipe lines cannot acquire a binding", async () => {
  const { glen } = await setup()
  const recipeId = await glen.mutation(api.recipes.create, { name: "Soup", tags: [], ingredients: [{ text: "salt and pepper" }] })
  await expect(glen.mutation(api.pantry.setRecipePresence, { recipeId, text: "salt and pepper", present: true, names: ["salt", "dhania"] })).rejects.toThrow("specific")
  expect((await glen.query(api.pantry.page, firstPage)).page).toEqual([])
  await expect(glen.mutation(api.pantry.setRecipePresence, { recipeId, text: "gone", present: true, names: ["salt"] })).rejects.toThrow("changed")
  await expect(glen.mutation(api.pantry.setPresence, { name: "a".repeat(121), present: true })).rejects.toThrow("120")
})
