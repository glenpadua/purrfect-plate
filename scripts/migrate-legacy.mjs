...
function run(name, args) {
  const raw = execFileSync(
    "pnpm",
    ["exec", "convex", "run", name, JSON.stringify(args), "--prod"],
...
for (const row of rows) {
  ...
  if (run("migrations:existing", { legacyId: _id })) {
    console.log("Already migrated:", content.name);
    continue;
  }
  ...
  if (imagePath && !photo) {
    ...
    for (const [size, quality] of [
      ...
      bytes = await sharp(await readFile(imagePath), { limitInputPixels: 25000000 })
        .rotate()
  ...
  run("migrations:addRecipe", {
    ...content,
    legacyId: _id,
    ...(photo ? { imageStorageId: photo.storageId } : {}),
  });
  console.log(
    "Migrated:",
    content.name,
    photo ? `${Math.ceil(photo.bytes / 1000)} KB` : "no photo",
  );
}
