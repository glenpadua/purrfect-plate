  ...
  while (!stopping) {
    try {
      ...
      if (job) {
        ...
        const response = await POST(new Request("http://localhost/api/internal/imports", {
          method: "POST", headers: { authorization: `Bearer ${secret}`, "content-type": "application/json" }, body: JSON.stringify(job),
        }));
        console.log(`Import ${job.id}: ${response.ok ? (await response.json()).status : `worker HTTP ${response.status}`}.`);
        continue;
      }
    } catch {
      // Never print provider errors that could embed request headers or keys.
      console.error("Local import worker could not reach the development queue. Check backend sync, network and worker secret.");
    }
...
void main().catch(() => { console.error("Local worker startup failed. Check development configuration."); process.exitCode = 1; });
