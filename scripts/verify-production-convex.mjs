...
const spec = JSON.parse(
  execFileSync("pnpm", ["exec", "convex", "function-spec", "--prod"], {
    encoding: "utf8",
...
assert.equal(
  spec.url,
  "https://spotted-gazelle-950.convex.cloud",
...
for (const [name, type] of Object.entries(required)) {
  assert.equal(
    functions.get(name)?.functionType,
    type,
    `Missing or incompatible production function: ${name}`,
  );
  assert.equal(
    functions.get(name)?.visibility.kind,
    "public",
    `Client function is not public: ${name}`,
  );
}
...
console.log(
  `${Object.keys(required).length} required function contracts present; obsolete pantry functions absent.`,
);
console.log(
  "This verifies the deployed API contract; also check deployment completion and signed-in hosted behavior.",
);
