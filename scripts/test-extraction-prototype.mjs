...
for (const url of urls) {
  const response = await fetch(`${base}/api/extraction-prototype`, {
    ...
    body: JSON.stringify({ url, useProvider, includeTranscript: true }),
  ...
  while (job.status === "running" && Date.now() < deadline) {
    ...
    const status = await fetch(`${base}/api/extraction-prototype?id=${job.id}`, {
      signal: AbortSignal.timeout(30000),
    });
    if (!status.ok)
      throw new Error(
        `Run ${job.id} could not be observed (HTTP ${status.status}); do not restart it without checking the server.`,
      );
  ...
  const result = {
    url,
    status: job.status,
    title: job.recipe?.title || job.title,
    ingredients: job.recipe?.ingredients.length ?? 0,
    steps: job.recipe?.steps.length ?? 0,
    evidence: job.evidence.map(({ kind, text, via }) => ({ kind, characters: text.length, via })),
    errors: job.attempts.filter((a) => a.outcome === "error" || a.outcome === "skipped"),
    providerCredits: job.providerCredits,
    aiUsage: job.aiUsage,
    jobId: job.id,
  };
  results.push(result);
...
await writeFile(
  ...
  JSON.stringify({ testedAt: new Date().toISOString(), base, useProvider, results }, null, 2),
);
