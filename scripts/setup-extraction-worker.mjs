...
for (const [command, args] of [
  ...
  [path.join(environment, 'bin', 'python'), ['-m', 'pip', 'install', '-r', path.join(root, 'scripts/extraction-worker/requirements.txt')]],
]) {
...
if (!/^EXTRACTION_PYTHON=/m.test(existing)) await writeFile(envFile, `${existing.trimEnd()}\nEXTRACTION_PYTHON=${JSON.stringify(path.join(environment, 'bin', 'python'))}\n`)
