...
export async function POST(request: Request) {
  ...
  try {
    const result = await importRecipe(
      job.url,
      (phase) =>
        client.mutation(api.imports.workerProgress, { ...args, phase }).then(() => undefined),
      { relevanceOverride: job.relevanceOverride },
    );
    ...
    await client.mutation(api.imports.workerFinish, {
      ...args,
      draft: result.draft,
    ...
    const message =
      error instanceof z.ZodError
        ? "We read the source, but could not organise it into a valid recipe. Try importing again."
        : error instanceof ImportSourceError
          ? error.message
          : error instanceof Error && /source|recipe information|link/i.test(error.message)
            ? error.message.slice(0, 300)
            : "The recipe could not be imported. Check the link and try again.";
    await client.mutation(api.imports.workerFinish, {
      ...args,
      error: message,
      ...(error instanceof ImportSourceError
        ? {
            ...
            ...(error.audit ? { evidenceJson: JSON.stringify(error.audit) } : {}),
            ...(error.searchQuery ? { searchQuery: error.searchQuery } : {}),
          }
        : {}),
    });
