...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (bytes[0] !== 0xff || bytes[1] !== 0xd8 || bytes[2] !== 0xff) return new Response("Invalid JPEG", { status: 400 })
    ...
    const response = await fetch(url, { method: "POST", headers: { "Content-Type": "image/webp" }, body: new Uint8Array(optimized) })
