...
export async function GET(request: Request) {
  if (process.env.VERCEL)
    return json({ error: "The local test bench is not available here." }, 404);
    ...
    if (job && new URL(request.url).searchParams.get("download") === "1") {
      return new Response(JSON.stringify(job, null, 2), {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="recipe-extraction-${job.platform}-${job.id}.json"`,
        },
      });
    }
    return job
      ...
      : json(
          {
            error:
              "This local run is no longer available. The server may have restarted or the run expired.",
          },
          404,
        );
  }
  return json({
    configuration: {
      localMedia: localMediaReady(),
      supadata: Boolean(process.env.SUPADATA_API_KEY),
      openai: Boolean(process.env.OPENAI_API_KEY),
      model: process.env.RECIPE_EXTRACTION_MODEL || "gpt-4.1-mini",
    },
    jobs: recentJobs(),
  });
}
