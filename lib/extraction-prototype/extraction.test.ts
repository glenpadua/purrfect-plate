...
import {
  metadataEvidence,
  preserveVerifiedSections,
  recipeFromSelections,
  selectablePassages,
  supplementRecipe,
  transcriptContent,
  verifyRecipe,
...
describe("URL boundary", () => {
  ...
  it.each([
    ...
    "::1",
    "192.0.2.1",
  ])("blocks non-public DNS answer %s", (ip) => expect(isPublicAddress(ip)).toBe(false));
  it("accepts public HTTPS and keeps the selected carousel slide", () =>
    expect(publicUrl("https://www.instagram.com/p/abc/?img_index=4#x").href).toBe(
      "https://www.instagram.com/p/abc/?img_index=4",
    ));
  it("does not misclassify lookalike domains as Instagram", () =>
    expect(platformFor(new URL("https://instagram.com.evil.example/p/test"))).toBe("website"));
});
...
describe("source parsing", () => {
  it("reads nested Recipe JSON-LD and preserves HowToSection ordering", () => {
          ...
          recipeInstructions: [
            {
              ...
              itemListElement: [
                { text: "Boil the water." },
                { text: "Add rice; simmer 15 minutes." },
              ],
            },
          ],
          recipeYield: 2,
    ...
    expect(result.recipe?.steps.map((s) => s.text)).toEqual([
      "Boil the water.",
  ...
  it("does not treat a title or description as a recipe", () => {
    const result = parsePage(
      '<title>Perfect pasta</title><meta property="og:description" content="Comment RECIPE for details">',
      "instagram",
    );
  ...
  it("copies explicit caption sections and strips trailing hashtags", () => {
    const evidence: Evidence = {
      ...
      kind: "caption",
      via: "test",
      text: '5 likes - chef on September 7: "Breakfast burritos\nIngredients:\n2 eggs\nSalt\nDirections:\n1. Whisk eggs.\n2. Cook in a pan.\n\n#breakfast".',
    };
  ...
  it("parses embedded braces inside strings without evaluating JavaScript", () =>
    expect(
      embeddedObject('var data = {"text":"a } \\" b", "n":1}; alert(1)', "var data =")?.n,
    ).toBe(1));
  it("preserves an ingredients-only caption as incomplete", () => {
    const recipe = recipeFromCaption({
      id: "caption",
      kind: "caption",
      via: "test",
      text: "Beef stir fry\nIngredients:\n1 tsp oil\n1 lb beef\n-\nTo serve:\nRice\n-\nMacros:\n400 calories\n#recipe",
  ...
  it("does not count an empty YouTube description or generic TikTok metadata as retrieved evidence", () => {
    expect(
      parsePage(
        'var ytInitialPlayerResponse = {"videoDetails":{"shortDescription":""}};',
        "youtube",
      ).evidence,
    ).toEqual([]);
    expect(
      parsePage('<meta property="og:description" content="TikTok">', "tiktok").evidence,
    ).toEqual([]);
  });
  ...
  it("handles native transcripts and rejects empty successful responses", () => {
    ...
    expect(parseTranscript("<transcript><text>Boil &amp; stir</text></transcript>")).toBe(
      "Boil & stir",
    );
  ...
  it("does not scrape navigation or scripts into website evidence", () => {
    const page = parsePage(
      "<nav>Buy ads</nav><main><h1>Soup</h1><p>Add water.</p><script>secret()</script></main>",
      "website",
    );
...
describe("provider evidence", () => {
  it("keeps directly extracted ingredients when AI returns none after verification", () => {
    ...
    const ai = {
      ...
      steps: [],
      servings: null,
      warnings: ["Unsupported ingredient removed."],
    };
  ...
  it("adds source-backed AI steps while retaining directly extracted ingredient lines", () => {
    ...
    const result = supplementRecipe(
      { title: "Stir fry", ingredients: [ingredient], steps: [], servings: null, warnings: [] },
      { title: "Stir fry", ingredients: [], steps: [step], servings: null, warnings: [] },
    );
  ...
  it("does not send videos in a mixed carousel to image text recognition", () => {
    const result = metadataEvidence({
      type: "carousel",
      media: {
        items: [
          { type: "video", url: "https://example.com/video.mp4" },
          { type: "image", url: "https://example.com/slide.jpg" },
        ],
      },
    });
  ...
  it("handles both documented asynchronous transcript shapes and silence", () => {
    ...
    expect(
      transcriptContent({ status: "completed", result: { content: [{ text: "Simmer." }] } }),
    ).toBe("Simmer.");
  ...
  it("removes invented ingredients, changed quantities, and unsupported servings", () => {
    const evidence: Evidence[] = [
      { id: "caption", kind: "caption", text: "2 eggs. Whisk the eggs.", via: "test" },
    ];
    const result = verifyRecipe(
      {
        ...
        ingredients: [
          ...
          { text: "Salt", quote: "Salt", sourceId: "caption" },
        ],
        steps: [{ text: "Whisk the eggs.", quote: "Whisk the eggs.", sourceId: "caption" }],
        servings: "4 people",
        warnings: [],
      },
      evidence,
  ...
  it("accepts cosmetic bullet removal while rejecting empty or altered evidence", () => {
    const result = verifyRecipe(
      ...
      [{ id: "caption", kind: "caption", text: "• 2 eggs", via: "test" }],
    );
    ...
    const passages = selectablePassages([
      {
        id: "speech",
        kind: "transcript",
        text: "Crack two\neggs. Buy my book. Whisk them.",
        via: "test",
      },
      { id: "visual", kind: "visual_observation", text: "Maybe add cumin.", via: "test" },
    ]);
    const result = recipeFromSelections(
      {
        ingredients: ["speech:0", "fake"],
        steps: ["speech:0", "speech:2", "speech:2", "visual:0"],
      },
      passages,
      { title: "Eggs", ingredients: [], steps: [], servings: null, warnings: [] },
    );
  ...
  it("keeps a fully verified ingredient list when only the method needs repair", () => {
    ...
    const repaired = {
      ...candidate,
      ingredients: [{ ...ingredient, text: "Entire duplicated caption" }],
    };
