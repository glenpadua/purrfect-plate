...
describe("source history", () => {
  ...
  it("groups the same Instagram post while preserving meaningful website queries", () => {
    expect(sourceKey("https://www.instagram.com/reel/abc/?utm_source=x")).toBe(sourceKey("https://instagram.com/p/abc/?img_index=4"))
    expect(sourceKey("https://example.com/recipe?id=1")).not.toBe(sourceKey("https://example.com/recipe?id=2"))
  })
})
