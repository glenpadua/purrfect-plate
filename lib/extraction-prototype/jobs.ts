...
const globalStore = globalThis as typeof globalThis & { recipeExtractionJobs?: Map<string, ExtractionJob> }
...
export function recentJobs() {
  return [...jobs.keys()].reverse().map((id) => getJob(id)!).map(({ id, url, platform, status, phase, title, recipe, startedAt, finishedAt }) => ({ id, url, platform, status, phase, title: recipe?.title || title?.split(" on Instagram:")[0], startedAt, finishedAt }))
}
export function startJob(input: ExtractionInput) {
  ...
  if (running.length >= 3) throw new Error("Three imports are already running. Wait for one to finish.")
  for (const [id, job] of jobs) {
    if (job.status !== "running" && (jobs.size >= 30 || Date.now() - Date.parse(job.startedAt) > 3_600_000)) jobs.delete(id)
  }
  const job: ExtractionJob = {
    id: randomUUID(), url: url.href, platform: platformFor(url), startedAt: new Date().toISOString(),
    status: "running", phase: "Starting", evidence: [], attempts: [], warnings: [], providerCredits: null,
  }
  jobs.set(job.id, job)
