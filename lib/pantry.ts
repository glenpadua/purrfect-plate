...
for (const [canonical, names] of Object.entries({
  ...
  "coriander leaves": ["cilantro", "fresh cilantro", "fresh coriander", "dhania leaves", "dhaniya leaves"],
  ...
  "clove": ["cloves", "laung"],
  "ginger": ["adrak"], "garlic": ["lahsun", "lasoon"],
  "potato": ["aloo", "alu"], "eggplant": ["brinjal", "baingan"],
...
export const ambiguousIngredientNames = new Set(["dhania", "dhaniya", "coriander", "methi", "haldi", "turmeric", "cumin", "elaichi", "dal", "chana", "moong", "urad"])

export function normalizeIngredientName(text: string) {
  return text.replace(/([¼½¾⅓⅔⅛⅜⅝⅞])/g, " $1").normalize("NFKC").replace(/⁄/g, "/").trim().toLowerCase().replace(/\s+/g, " ")
}

export const ingredientSuggestions = [...new Set([...Object.keys(ingredientAliases), ...Object.values(ingredientAliases)])].sort()
...
export function ingredientIdentity(text: string): { key: string; name: string } | null {
  ...
  if (ambiguousIngredientNames.has(name) || !name || name.length > 120 || /\b(?:and|or)\b|[\d/&(),;:–—]|\n/.test(name) || !/^[\p{L}\p{M}][\p{L}\p{M}\s'-]*$/u.test(name)) return null
