...
export function checkImportTarget(value: string) {
  ...
  if (host === "youtube.com" || host === "youtu.be") {
    ...
    specific =
      /^\/@[\w.-]+\/video\/\d+\/?$/.test(url.pathname) || /^\/t\/[\w-]+\/?$/.test(url.pathname);
  if (!specific)
    throw new ImportSourceError(
      "Share a specific recipe page, post or video. Profiles, feeds, search pages and playlists cannot be imported.",
      "wrong_link",
    );
  if (/\.(?:pdf|zip|exe|dmg|mp[34]|mov|jpg|png|webp)$/i.test(url.pathname))
    throw new ImportSourceError(
      "Share the recipe page or original social post instead of a direct file link.",
      "wrong_link",
    );
}

const decisionSchema = z.object({
  classification: z.enum(["recipe", "cooking", "technique", "unrelated", "unknown"]),
  quote: z.string().max(500),
  dish: z.string().max(80).nullable(),
});
...
export function readPreflightDecision(raw: unknown, source: string) {
  ...
  if (
    (decision.classification === "unrelated" || decision.classification === "technique") &&
    (source.length > 8000 ||
      decision.quote.trim().length < (decision.classification === "technique" ? 12 : 20) ||
      !normalized(source).includes(normalized(decision.quote)))
  )
    decision.classification = "unknown";
  if (decision.dish && !normalized(source).includes(normalized(decision.dish)))
    decision.dish = null;
  return decision;
}
