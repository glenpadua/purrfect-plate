...
export function mediaHeaders(url: URL, session?: MediaSession, now = Date.now()) {
  ...
  const cookies = session.cookies.filter((c) => {
    ...
    const hostMatches =
      url.hostname === domain || (!c.hostOnly && url.hostname.endsWith(`.${domain}`));
    const pathMatches =
      url.pathname === c.path ||
    ...
    return (
      ...
      (c.expires == null || c.expires * 1000 > now) &&
      /^[!#$%&'*+.^_`|~\w-]+$/.test(c.name) &&
      !/[;\r\n]/.test(c.value)
    );
  });
...
export function publicUrl(input: string) {
  ...
  if (
    !host.includes(".") ||
    /\.(local|localhost|internal|test|invalid)$/i.test(host) ||
...
export async function fetchPublicBytes(
  ...
  signal = AbortSignal.timeout(maxBytes > 3_000_000 ? 45000 : 20000),
): Promise<{ bytes: Buffer; url: string; status: number; contentType: string }> {
  ...
  if (!addresses.length || addresses.some(({ address }) => !isPublicAddress(address)))
    throw new Error("The link resolves to a non-public address.");
  ...
  const result = await new Promise<{
    ...
    const req = request(
      url,
      {
        ...
        headers: {
          "User-Agent": "Mozilla/5.0 (compatible; PurrfectPlatePrototype/0.1)",
          Accept: "text/html,application/json,text/xml;q=0.9,*/*;q=0.1",
          "Accept-Encoding": "identity",
          ...mediaHeaders(url, session),
        },
        signal,
      },
      (response) => {
        ...
        response.on("data", (chunk: Buffer) => {
          ...
          if (bytes > maxBytes) {
            response.destroy(
              new Error(
                `Source exceeds the prototype's ${Math.round(maxBytes / 1_000_000)} MB limit.`,
              ),
            );
            return;
          }
