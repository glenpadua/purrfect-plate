...
export type Attempt = {
  stage: string;
  outcome: "ok" | "unavailable" | "error" | "skipped";
  detail: string;
  elapsedMs: number;
};
const sourcedText = z
...
export const extractionInput = z
  .object({
...
export function recipeGaps(recipe: ExtractedRecipe) {
  return [
    !recipe.ingredients.length && "The source did not provide an ingredient list.",
    !recipe.steps.length && "The source did not provide cooking instructions.",
    recipe.warnings.some((w) => /^An unsupported (ingredient|step) was removed/.test(w)) &&
      "Some proposed recipe content could not be verified; this result needs review.",
