...
export async function importRecipe(url: string, progress: (phase: string) => Promise<void>, options: { relevanceOverride?: boolean } = {}) {
  checkImportTarget(url)
  const deadline = Date.now() + 260000
  const budget = (maximum: number, reserve = 0) => Math.max(1, Math.min(maximum, deadline - Date.now() - reserve))
  ...
  try {
    ...
    if (page.status !== 200 || !page.contentType.includes("text/html")) throw new Error("Source page unavailable")
    finalUrl = page.url
    if (/\/(?:accounts\/login|login|signin|consent)(?:\/|$)/i.test(new URL(finalUrl).pathname)) {
      if (platform !== "website") { finalUrl = url; throw new Error("The public page requires sign-in; trying the original post through the media adapter.") }
      throw new ImportSourceError("This source requires access we do not have. Try a public recipe page or post.", "unavailable")
    }
  ...
  } catch (error) { if (error instanceof ImportSourceError) throw error; warnings.push("The public page could not be read directly.") }
  ...
  if (platform !== "website" && process.env.MEDIA_WORKER_URL && process.env.MEDIA_WORKER_SECRET) {
    try {
      const response = await fetch(`${process.env.MEDIA_WORKER_URL}/metadata`, { method: "POST", headers: { Authorization: `Bearer ${process.env.MEDIA_WORKER_SECRET}`, "Content-Type": "application/json" }, body: JSON.stringify({ url: finalUrl, includeTranscript: true }), signal: AbortSignal.timeout(budget(30000, 105000)) })
      if (response.ok) {
        const metadata = metadataSchema.parse(await response.json())
        if (metadata.duration && metadata.duration > 600) throw new ImportSourceError("Videos longer than ten minutes are not supported yet. Share a shorter clip or the creator’s recipe page.", "unavailable")
        sourceTitle = metadata.title || sourceTitle
        for (const [id, kind, text] of [["preflight-caption", "caption", metadata.caption], ["preflight-transcript", "transcript", metadata.transcript]] as const) {
          if (text && !evidence.some(item => item.text === text)) evidence.push({ id, kind, text, via: kind === "caption" ? "Public post caption" : "Existing video subtitles" })
        }
      }
    } catch (error) { if (error instanceof ImportSourceError) throw error /* Unreadable metadata is unknown, not a rejection. */ }
  }
  const completeCaption = evidence.some(item => item.kind === "caption" && (() => { const recipe = recipeFromCaption(item); return !!recipe?.ingredients.length && !!recipe.steps.length })())
  ...
  if (platform !== "website" && !completeCaption) {
    ...
    try {
      ...
      const response = await fetch(`${workerUrl}/analyze`, { method: "POST", headers: { Authorization: `Bearer ${secret}`, "Content-Type": "application/json" }, body: JSON.stringify({ url: finalUrl, includeTranscript: true, includeFrames: true }), signal: AbortSignal.timeout(budget(150000, 105000)) })
      ...
      evidence.push(...media.evidence.filter(item => !evidence.some(existing => existing.text === item.text && existing.kind === item.kind)))
      if (media.images?.length) {
        sourceImageUrl = media.images[0].url
        await progress("Reading recipe text in the photos")
        for (const item of media.images) {
          if (budget(90000, 105000) < 15000) { warnings.push("Some photos could not be read within this import’s time limit. Check the original for omitted details."); break }
          try {
            const bytes = await retrieveRecipeImage(item.url, AbortSignal.timeout(budget(20000, 105000)))
            const result = await readImageText(`data:image/webp;base64,${bytes.toString("base64")}`, budget(90000, 105000))
            if (!image) image = bytes
            if (result.text.trim() !== "[NO READABLE TEXT]") evidence.push({ id: `slide-${item.index}`, kind: "image_text", text: result.text, via: `Text read from slide ${item.index}; check against original` })
          } catch { warnings.push(`Slide ${item.index} could not be read.`) }
        }
      }
      if (media.frames.length) {
        ...
        try { image ??= await optimizeRecipeImage(Buffer.from(frame.imageUrl.split(",")[1], "base64")) } catch { /* Placeholder is an intentional fallback. */ }
        if ((!media.transcript || !/cook|stir|add|mix|bake|fry|chop|boil/i.test(media.transcript)) && budget(90000, 105000) >= 5000) {
          ...
          for (const [index, block] of observations.printedText.entries()) evidence.push({ id: `frame-${index}`, kind: "image_text", text: block.text, via: `Text read near ${block.frame}; check against original` })
          if (observations.observations.length) evidence.push({ id: "visual-observations", kind: "visual_observation", text: observations.observations.join("\n"), via: "Unverified visual observations; not recipe facts" })
        }
      }
    } catch { mediaUnavailable = true; warnings.push("Some video or photo content could not be retrieved from this platform.") }
  }
  ...
  return { draft: result.draft, image, author, evidenceJson: JSON.stringify({ version: 1, url, finalUrl, platform, extractedAt: new Date().toISOString(), evidence, citations: result.citations, usage: result.usage, mediaUsage, preflight, warnings }) }
}
