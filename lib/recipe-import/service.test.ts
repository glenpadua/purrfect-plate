...
test("empty normalized recipes retain model usage and citations, with oversized diagnostics explicitly bounded", async () => {
  ...
  fetch.mockImplementation(async (url, options) => {
    ...
    return Response.json({ status: "completed", output: [{ content: [{ type: "output_text", text: JSON.stringify(preflight
      ...
      : { name: "Biryani", contentType: "unknown", classificationQuote: "", tags: [], ingredients: [], instructions: [], servings: null, warnings: ["The only readable text was a creator watermark."] }) }] }], usage: { input_tokens: preflight ? 200 : 700, output_tokens: preflight ? 30 : 90 } })
  })
...
test("an explicitly technique-only title stops before costly video analysis", async () => {
  ...
  fetch.mockImplementation(async (url, options) => {
    if (url !== "https://api.openai.com/v1/responses") return network(url, options)
    return Response.json({ status: "completed", output: [{ content: [{ type: "output_text", text: JSON.stringify({ classification: "technique", quote: title, dish: null }) }] }], usage: { input_tokens: 210, output_tokens: 40 } })
  })
  const failure = await importRecipe("https://youtube.com/shorts/z1XshOQJmrw", async () => {}).catch(error => error)
  expect(failure).toBeInstanceOf(ImportSourceError)
  expect(failure.code).toBe("not_recipe")
  expect(failure.message).toContain("cooking technique rather than a recipe")
  expect(failure.audit.preflight).toMatchObject({ classification: "technique", quote: title, usage: { inputTokens: 210, outputTokens: 40 } })
  expect(fetch.mock.calls.map(([url]) => url)).toEqual(["https://media.example.com/metadata", "https://api.openai.com/v1/responses"])
})
...
test("a wrapping tutorial discovered in the real transcript fails even when generic ingredients could be extracted", async () => {
  ...
  const transcript = "I'm going to show you how to roll the perfect burrito. Your first step is to heat up the tortilla to make it more pliable. Microwave or place it directly on a gas burner for about 30 seconds, turning constantly. Place down the warm tortilla, then place all your fillings directly into the center. You're then going to want to fold in both the sides, then roll the bottom of the burrito. Now, this is the most important part. Tuck the filling and fold in the corners. Then roll it up and heat it on the outside to seal it all together."
  ...
  fetch.mockImplementation(async (url, options) => {
    ...
    return Response.json({ status: "completed", output: [{ content: [{ type: "output_text", text: JSON.stringify({
      name: "How to wrap a burrito", contentType: "technique", classificationQuote: "I'm going to show you how to roll the perfect burrito.", tags: [], servings: null, warnings: [],
      ingredients: [{ text: "Tortilla", passageIds: ["social-transcript:1"] }, { text: "Fillings", passageIds: ["social-transcript:3"] }],
      instructions: [{ text: "Fold in both the sides, then roll the bottom of the burrito.", passageIds: ["social-transcript:4"] }],
  ...
  expect(failure.message).toContain("cooking technique rather than a recipe")
  expect(failure.audit).toMatchObject({
    usage: { inputTokens: 800, outputTokens: 120 }, mediaUsage: { inputTokens: 5100 },
    citations: { selection: { contentType: "technique", classificationQuote: "I'm going to show you how to roll the perfect burrito." } },
  })
...
test.each([
  { title: "Breakfast CrunchWrap", text: "In a cold pan lay out a generous amount of beef bacon. Now in a bowl I want you to crack in a bunch of eggs.", ingredient: "Beef bacon", step: "Lay the beef bacon in a cold pan." },
  { title: "How to wrap a chicken burrito", text: "For this burrito, cook chicken with cumin and salt. Add cooked rice and the chicken to a tortilla, then fold and wrap it.", ingredient: "Chicken", step: "Cook chicken with cumin and salt." },
])("allows source-backed dish preparation despite missing amounts or wrapping language: $title", async ({ title, text, ingredient, step }) => {
  ...
  fetch.mockImplementation(async (url, options) => {
    ...
    return Response.json({ status: "completed", output: [{ content: [{ type: "output_text", text: JSON.stringify(preflight
      ...
      : { name: title, contentType: "recipe", classificationQuote: "", tags: [], servings: null, warnings: ["Ingredient amounts were not stated."], ingredients: [{ text: ingredient, passageIds: ["preflight-transcript:0"] }], instructions: [{ text: step, passageIds: ["preflight-transcript:0"] }] }) }] }], usage: { input_tokens: 300, output_tokens: 70 } })
  })
