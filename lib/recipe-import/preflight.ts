...
export async function preflightRecipe(source: string) {
  ...
  if (bounded.trim().length < 12)
    return { classification: "unknown" as const, quote: "", dish: null, usage: undefined };
  try {
          ...
          format: {
            ...
            schema: {
              ...
              properties: {
                classification: {
                  type: "string",
                  enum: ["recipe", "cooking", "technique", "unrelated", "unknown"],
                },
                quote: { type: "string" },
                dish: { type: ["string", "null"] },
              },
              required: ["classification", "quote", "dish"],
            },
          },
        },
      },
