...
const passageLine = z.object({
  text: z.string().min(1).max(3000),
  passageIds: z.array(z.string()).min(1).max(MAX_SOURCE_PASSAGES),
});
const passageOutput = z.object({
  name: z.string().min(1).max(200),
  tags: z.array(z.string().max(40)).max(20),
  ingredients: z.array(passageLine).max(100),
  instructions: z.array(passageLine).max(100),
  servings: z.string().max(100).nullable(),
  warnings: z.array(z.string().max(500)).max(20),
});
const normalizationOutput = passageOutput.extend({
  contentType: z.enum(["recipe", "technique", "unrelated", "unknown"]),
  classificationQuote: z.string().max(500),
});

export function draftFromPassages(
  raw: z.infer<typeof passageOutput>,
  passages: ReturnType<typeof selectablePassages>,
) {
  const lookup = new Map(passages.map((p) => [p.id, p]));
  const warnings = [...raw.warnings];
  const convert = (items: z.infer<typeof passageLine>[]) =>
    items.map((item) => {
      const selected = [...new Set(item.passageIds)].map((id) => lookup.get(id));
      if (!selected.length || selected.some((p) => !p))
        throw new Error("Recipe references could not be verified. Please retry the import.");
      ...
      if (unsupportedNumber)
        warnings.push(
          "A step or ingredient uses the original source wording so its amounts stay accurate.",
        );
      const text = unsupportedNumber ? quote : item.text.trim();
      if (text.length > 3000)
        throw new Error(
          "A source passage was too long to save safely. Try the creator’s recipe page.",
        );
  ...
  if (!ingredients.length)
    warnings.push("No ingredient list was available. Add it before cooking.");
  if (!instructions.length)
    warnings.push("No cooking method was available. Add it before cooking.");
  const servings =
    raw.servings && passages.some((p) => squash(p.text).includes(squash(raw.servings!)))
      ? raw.servings
      : undefined;
  return {
    name: raw.name,
    tags: canonicalizeRecipeTags(raw.name, raw.tags),
    ingredients,
    instructions,
    servings,
...
export async function normalizeRecipe(evidence: Evidence[], timeoutMs = 90000) {
  ...
  if (publisher) {
    ...
    const parsed = parsePage(
      `<script type="application/ld+json">${publisher.text.replace(/<\//g, "<\\/")}</script>`,
      "website",
    );
    ...
    if (recipe?.ingredients.length && recipe.steps.length) {
      ...
      const card = cardEvidence
        ? publisherCardSchema.parse(JSON.parse(cardEvidence.text))
        : undefined;
      const recipeNotes = card?.notes.map((text) => ({ text, sourceIds: [cardEvidence!.id] }));
      const notesMissing = missingPublisherNotes(
        [...recipe.ingredients, ...recipe.steps].map((line) => line.text),
        card?.notes,
      );
      return {
        ...
        draft: {
          name: recipe.title,
          tags: canonicalizeRecipeTags(recipe.title, []),
          ingredients: card
            ? card.ingredients.map((item) => ({
                ...item,
                sourceIds: [publisher.id, cardEvidence!.id],
              }))
            : recipe.ingredients.map((l) => ({ text: l.text, sourceIds: [publisher.id] })),
          instructions: recipe.steps.map((l) => ({ text: l.text, sourceIds: [publisher.id] })),
          servings: recipe.servings ?? undefined,
          ...(recipeNotes?.length ? { recipeNotes } : {}),
          warnings: notesMissing
            ? parsed.warnings
            : parsed.warnings.filter((w) => !w.startsWith("This recipe refers to notes")),
        },
  ...
  const source = evidence
    ...
    .map((e) => ({ ...e, text: e.text.slice(0, 60000) }));
  if (!source.some((e) => e.text.trim()))
    throw new Error("We could not read this source. Check that the link is public and try again.");
  const passages = selectablePassages(source);
  if (passages.length > MAX_SOURCE_PASSAGES)
    throw new Error("This source is too long to normalize reliably. Try a shorter recipe link.");
  const itemSchema = {
    type: "object",
    ...
    properties: {
      ...
      passageIds: {
        type: "array",
        minItems: 1,
        maxItems: MAX_SOURCE_PASSAGES,
        items: { type: "string", enum: passages.map((p) => p.id) },
      },
    },
    required: ["text", "passageIds"],
  };
        ...
        format: {
          ...
          schema: {
            ...
            properties: {
              ...
              contentType: {
                type: "string",
                enum: ["recipe", "technique", "unrelated", "unknown"],
              },
              classificationQuote: { type: "string" },
              ingredients: { type: "array", items: itemSchema },
              instructions: { type: "array", items: itemSchema },
  ...
  if (
    (contentType === "technique" || contentType === "unrelated") &&
    (squash(raw.classificationQuote).length < 12 ||
      !passages.some((passage) => squash(passage.text).includes(squash(raw.classificationQuote))))
  )
    contentType = "unknown";
  return {
    contentType,
    draft: draftFromPassages(raw, passages),
    citations: { passages, selection: raw },
