...
import { extractIngredientQuantity, ingredientForCooking, ingredientScale, parseIngredient, withIngredientQuantity } from "./cooking"
const half = (text: string) => ingredientForCooking(withIngredientQuantity({ text }), { factor: 0.5, units: "original" })
...
test("source quantities become persistent records for counts, ranges and alternative weights", () => {
  ...
  expect(line).toMatchObject({ text, sourceIds: ["publisher"], quantity: { version: 1, sourceText: text, status: "scalable", parsed: { amount: 5, alternate: { amount: 400, unit: "gms" } } } })
...
test("range anchors and unsafe multi-amount interpretations are rejected", () => {
  for (const text of ["1 (400 g) can tomatoes", "2 x 400g cans beans", "2 cans tomatoes (400 g)", "2 inch ginger", "5% cream", "Salt- 4/0 tsp", "Oil- 6–5 tbsp", "2 eggs plus 1 yolk"]) {
...
test("corrections survive JSON storage, preserve the source, and invalidate when source text changes", () => {
  ...
  const line = withIngredientQuantity({ text: original, quantity: extractIngredientQuantity(original, "2 lemons, juiced") })
  const reloaded = JSON.parse(JSON.stringify(line))
  expect(ingredientForCooking(reloaded, { factor: 0.5, units: "original" }).text).toBe("1 lemons, juiced")
...
test("explicit added portions scale separately without losing their preparation notes", () => {
  ...
  expect(half(text).text).toBe("Onions sliced- 2½ medium (200 g) + ½ medium to be made into a birista")
  expect(half("Mint leaves - around 10- 15 leaves").text).toBe("Mint leaves - around 5–7½ leaves")
  expect(half("Onion- 1 medium to be made into a birista").text).toBe("Onion- ½ medium to be made into a birista")
