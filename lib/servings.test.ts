...
test("invalid and ambiguous amounts are never silently multiplied", () => {
  for (const value of ["0", "-2", "1/0", "Infinity", "NaN", "1-2", "2 cups", "1e3", ""])
    expect(parseCookingAmount(value)).toBeNull();
  ...
  for (const factor of [0, -1, Infinity, NaN, 101])
    expect(ingredientForCooking("1 kg lamb", { factor, units: "original" }).unchanged).toBe(true);
  expect(ingredientForCooking("0.01 tsp salt", { factor: 0.1, units: "original" }).unchanged).toBe(
    true,
  );
});

test("source and user counts win, including after ingredient changes", () => {
  expect(resolveServings({ servings: "Serves 4", ingredients: [{ text: "700 g rice" }] })).toEqual({
    count: 4,
    origin: "source",
  });
  expect(
    resolveServings({
      ...
      ingredients: [{ text: "700 g rice" }],
    }),
  ).toEqual({ count: 6, origin: "user" });
  expect(resolveServings({ servings: "4–6 servings" })).toMatchObject({
    count: 5,
    origin: "estimated",
  });
});
...
test("old recipes get the same labeled, explainable estimates as imports", () => {
  expect(
    resolveServings({
      name: "Biryani",
      ingredients: [{ text: "Mutton- 1 kg" }, { text: "Rice- 700 gms (4 US cups)" }],
    }),
  ).toMatchObject({ count: 8, origin: "estimated", reason: expect.stringContaining("Rice") });
  expect(resolveServings({ name: "Omelette", ingredients: [{ text: "4 eggs" }] })).toMatchObject({
    count: 2,
    origin: "estimated",
  });
  expect(resolveServings({ ingredients: [{ text: "1 tbsp rice flour" }] })).toMatchObject({
    count: 4,
    origin: "estimated",
    reason: expect.stringContaining("rough starting"),
  });
