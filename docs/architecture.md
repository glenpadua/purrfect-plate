...
- Git integration is configured for `main` on the web and media Vercel projects. Web root is the repository root; media root is `services/media`. A real commit-triggered deployment is still to be verified; current changes remain uncommitted. Convex deployment remains a separate CLI step until a project deploy key is installed in the build environment.
- Resolve hosted YouTube retrieval. Direct cloud downloads/transcripts still fail for the supplied Shorts; the deployed Gemini 3.6 Flash adapter has read CrunchWrap narration successfully. Failed proof-of-origin diagnostics and vendor code have been removed from source; see [YouTube research](youtube-retrieval.md) for evidence, the direct-video API trial, and its acceptance status.
...
The recipe detail screen delegates cooking interaction to `CookingPanel`. It receives recipe values and never writes the adjusted quantities back to Convex. `ingredientForCooking` is the client-safe pure interface for serving and unit display; a native UI can reuse it without importing web components. Ambiguous ranges, package quantities and unspecified volume conventions stay as written. The original method and source amounts remain available. See [cooking behavior](cooking.md).
