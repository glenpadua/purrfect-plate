...
- Resolve remaining dependency advisories before public launch. On 13 September 2026, compatible transitive updates reduced `pnpm audit --prod` from 10 high, 10 moderate and 1 low to 0 critical/high/low and 4 moderate advisories. Remaining paths involve `uuid` and `stream-json` through Clerk wallet dependencies, and `decode-uri-component` through Expo Router. Their fixes require major dependency changes; do not force incompatible overrides. Recheck upstream releases and repeat native/web verification when updating. Isolate dependency installs from active dev sessions.
...
The only product UI is the Expo application in `apps/mobile`, targeting native iOS/Android and browsers with React Native Web. The folder name is historical. Expo Router route files compose the same feature screens for all targets; there are no parallel web/native copies of library, detail, cooking, editor, imports, pantry or account screens. `ui/product-shell.tsx` owns responsive navigation, and `ui/index.ts` exposes the shared layout, typography, controls and styles. Browser breakpoints change layout, not feature ownership.
...
## Maintainability pass (13 September 2026)

Feature screens consume their adjacent data hooks; hook/import lint rules enforce that convention. The shared visual controls and the guarded async-action hook have separate ownership. Manual editing and imported-draft review use the same pure form parser, retaining original source text and explicit time clearing. Backend pantry implementation is split by ingredient identity, temporary shopping, migration/rename and recipe matching, while public function names remain unchanged. Recipe deletion now removes its recipe-specific ingredient bindings as well as cooking preferences; shared pantry identities survive.

The root `pnpm check` and CI protect types, formatting, hooks, feature imports and transitive client-package dependencies. A built-web smoke check tests the actual gateway rather than only a rewrite configuration. Provider/extraction semantics, media retrieval, authentication, source provenance and production schema indexes were retained. Production deployment and device acceptance remain independently verifiable steps.

Local verification passed 162 domain/backend/architecture tests, 18 shared UI tests, nine media tests and three prototype-worker tests, plus lint, formatting, both TypeScript checks, the Expo/Next production build, built-web routing checks and an iOS bundle export. The signed-in inline browser passed search and pantry coverage, portion persistence after reload, cook-mode navigation, invalid editor input, pantry filtering/rename cancellation and shopping-list clipboard output. Pantry editing was inspected at 390px width. This pass also fixed a browser-only cook-mode exit error when screen wake locks are unavailable; Expo's cleanup-warning option allows cooking navigation to continue. These checks do not establish physical-device acceptance or production backend deployment.
