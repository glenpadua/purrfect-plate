...
const match = (text: string, present = false, resolved = true) => ({
  text,
  present,
  resolved,
  chosen: false,
...
test("one ingredient checkbox uses pantry presence and controls adding missing names", async () => {
  ...
  function Kitchen() {
    ...
    return (
      <PantryCookingView
        recipeId={recipeId}
        recipe={{ ingredients: matches.map((item) => ({ text: item.text })) }}
        matches={matches}
        setPresence={async (args) => {
          save(args);
          setMatches((items) =>
            items.map((item) =>
              item.text === args.text ? { ...item, present: args.present } : item,
            ),
          );
        }}
  ...
  expect(screen.getByRole("checkbox", { name: "2 onions" }).props.accessibilityState.checked).toBe(
    true,
  );
  await fireEvent.press(screen.getByRole("checkbox", { name: "100 g flour" }));
  expect(
    screen.getByRole("button", { name: "Add missing ingredients" }).props.accessibilityState
      .disabled,
  ).toBe(true);
...
test("an ambiguous line remembers its explicit recipe choice and can be changed", async () => {
  ...
  function Kitchen() {
    ...
    return (
      <PantryCookingView
        recipeId={recipeId}
        recipe={{ ingredients: [{ text: "butter or oil" }] }}
        matches={matches}
        addMissing={async () => 1}
        setPresence={async (args) => {
          save(args);
          setMatches([
            {
              ...
              resolved: true,
              chosen: true,
              names: args.names ?? matches[0].names,
            },
          ]);
        }}
      />
...
test("failed pantry confirmation stays unchecked and blocks shopping while saving", async () => {
  ...
  await fireEvent.press(screen.getByRole("checkbox"));
  expect(screen.getByRole("button", { name: "Saving…" }).props.accessibilityState.disabled).toBe(
    true,
  );
  await act(async () => {
...
test("portion changes keep the original pantry identity", async () => {
  ...
  await render(
    <PantryCookingView
      recipeId={recipeId}
      recipe={{ servings: "2", ingredients: [{ text: "100 g flour" }] }}
      matches={[match("100 g flour", true)]}
      setPresence={save}
      addMissing={async () => 0}
    />,
  );
  ...
  await fireEvent.press(screen.getByRole("button", { name: "Increase servings" }));
  expect(
    screen.getByRole("checkbox", { name: "200 g flour" }).props.accessibilityState.checked,
  ).toBe(true);
