...
jest.mock("../../lib/share-shopping", () => ({ shareShopping: async () => "Select and copy your list below." }));
const riceId = "rice" as Id<"pantryItems">;
const rice = { id: riceId, key: "rice", name: "rice", present: true, updatedAt: 1 };
const shopping = { id: "shopping" as Id<"shoppingItems">, key: "rice", name: "rice", ingredientId: riceId, createdAt: 1 };
function actions() {
  const calls = { setPresence: jest.fn(async () => riceId), addToShopping: jest.fn(async () => null), removeShopping: jest.fn(async () => null),
    clearShopping: jest.fn(async () => [{ name: "rice", ingredientId: riceId, createdAt: 1 }]), restoreShopping: jest.fn(async () => null),
    rename: jest.fn(async () => ({ status: "saved" as const })), renameShopping: jest.fn(async () => ({ status: "saved" as const })) };
...
test("clear and Undo restore shopping in order without changing pantry", async () => {
  const calls = actions();
  await render(<KitchenView items={[rice]} shopping={[shopping]} search="" onSearch={() => {}} actions={calls} />);
  ...
  expect(calls.restoreShopping).toHaveBeenCalledWith({ items: [{ name: "rice", ingredientId: riceId, createdAt: 1 }] });
...
test("rename previews a collision and requires explicit merge confirmation", async () => {
  const rename = jest.fn(async (_name: string, mergeInto?: Id<"pantryItems">) => mergeInto ? { status: "saved" as const } : { status: "merge_required" as const, targetId: riceId, targetName: "rice" });
