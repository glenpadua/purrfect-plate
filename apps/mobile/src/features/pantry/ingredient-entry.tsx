...
export function IngredientSuggestions({
  ...
  const names = [...new Set([...knownNames, ...ingredientSuggestions])]
    .filter(
      (name) => name.toLocaleLowerCase().includes(query) && name.toLocaleLowerCase() !== query,
    )
    .slice(0, 5);
  return (
    <View style={styles.row}>
      {names.map((name) => (
        <Button key={name} secondary title={name} onPress={() => onSelect(name)} />
      ))}
    </View>
  );
}
...
export function IngredientEntry({
  ...
  return (
    <View style={{ gap: 10 }}>
      ...
      />
      {!pending && !disabled && (
        <IngredientSuggestions value={name} knownNames={knownNames} onSelect={change} />
      )}
      <Button
        title="Add"
