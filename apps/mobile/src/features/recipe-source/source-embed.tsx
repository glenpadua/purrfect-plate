...
export function SourceEmbed({ source, name }: { source: Source; name: string }) {
  ...
  return (
    <>
      ...
      <Button
        secondary
        title={expanded ? "Hide source" : `Load ${source.provider} source`}
        onPress={() => setExpanded(!expanded)}
      />
      <Body muted>
        {expanded
          ? "Can’t play it here? Open the original above."
          : `Loads content from ${source.provider} when you tap. Nothing plays automatically.`}
      </Body>
    </>
  );
}
