...
jest.mock("./source-frame", () => ({ SourceFrame: ({ name }: { name: string }) => {
  const { Text } = require("react-native"); return <Text>{`Player for ${name}`}</Text>;
} }));
test("source players are mounted only after Load and removed on Hide", async () => {
  for (const url of ["https://youtube.com/shorts/z1XshOQJmrw", "https://www.instagram.com/reel/DdJyDKhKk1i/", "https://www.tiktok.com/@biteswithesther/video/7351594254663159083"]) {
