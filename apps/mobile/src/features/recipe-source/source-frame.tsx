...
export function SourceFrame({ source, name }: { source: Source; name: string }) {
  ...
  const appId = Constants.executionEnvironment === ExecutionEnvironment.StoreClient
    ? "host.exp.exponent"
    : Platform.OS === "ios" ? Constants.expoConfig?.ios?.bundleIdentifier : Constants.expoConfig?.android?.package;
  ...
  return <View style={{ width: "100%", minHeight: 200, aspectRatio: source.portrait || source.instagramUrl ? 9 / 16 : 16 / 9 }}>
    <WebView
      accessibilityLabel={`${source.provider} source for ${name}`}
      source={{ uri, headers: source.provider === "YouTube" && appId ? { Referer: `https://${appId.toLowerCase()}` } : undefined }}
      style={{ flex: 1 }}
