...
test("500 g mutton previews and applies a half batch with readable fractions", async () => {
  await render(<CookingPanel recipe={{ servings: "4", ingredients: [
    { text: "Mutton- 1 kg" }, { text: "Rice- 700 gms (4 US cup measure)" },
    { text: "Ginger Garlic paste- 2 tbsp" }, { text: "Turmeric Powder- 1/2 tsp" }, { text: "Salt to taste" },
  ] }} />);
...
test("estimated base servings can be corrected inline with validation and save failures", async () => {
  const save = jest.fn(async (_count: number) => { throw new Error("Offline"); });
...
test("saved adjustments survive a remount and stale ingredient anchors remain visible", async () => {
  const preference = { adjustment: { mode: "ingredient" as const, ingredientText: "Mutton- 1 kg", amount: 500, unit: "g" }, units: "original" as const };
  const { rerender } = await render(<CookingPanel recipe={{ servings: "4", ingredients: [{ text: "Mutton- 1 kg" }] }} preference={preference} />);
  expect(screen.getByText("Mutton- 500 g")).toBeTruthy();
  await rerender(<CookingPanel key="reopened" recipe={{ servings: "4", ingredients: [{ text: "Mutton- 1 kg" }] }} preference={preference} />);
  expect(screen.getByText("Mutton- 500 g")).toBeTruthy();
  await rerender(<CookingPanel recipe={{ servings: "4", ingredients: [{ text: "Mutton- 2 kg" }] }} preference={preference} />);
